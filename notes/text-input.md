...
**Mid-composition echoes are ignored.** While the IC's own outbound preedit is active (`activePreeditUtf16Length > 0`), `updateFromCompositor` drops non-authoritative reports entirely. They are the client echoing the edit we just sent, and Qt/KTextEditor (Kate) even include the preedit in the reported text — KTextEditor renders preedit by inserting it into the document buffer, so Qt's surrounding-text query sees it (violating the protocol's "excluding preedit" rule). Mirroring such an echo removed the Editable's composing span and pushed `IMM.updateSelection(…, -1, -1)` ("composition ended") at the IME; Gboard reacts to that with a defensive `finishComposingText`, force-committing every keystroke's preedit — typing "hello" in Kate yielded `he hel hell hello hello`. During composition the IME owns the editor model; genuine drift is reconciled by the report that follows composition end (commit/finish reset `activePreeditUtf16Length`). Coverage: `text_input::test_preedit_echo_client_*` against `wayland-debug-app text-input-echo-preedit`.

The compositor marks a report `authoritative` (must apply even mid-composition) when either:
- `cause=other` — the protocol's explicit out-of-band-change signal (user click, arrow keys); dropping the composition is then correct; or
- it belongs to an **enable cycle** (`just_enabled`, including the synthesized empty sync when the client enables without reporting). Enable resets all instance state, so an echo is structurally impossible — and the IC's preedit-active flag can be stale from a previous field, because text-input `leave`/`enable` reset only compositor-side state. Without this bit, a focus change to a new window mid-composition would suppress the fresh field's initial resync and a live IME could commit its in-flight word into the wrong window.

Known residual race (accepted): if the IME ends the composition while the echo of its last preedit update is still in flight, the flag is already 0 when the stale echo lands, so it is mirrored for one beat (including a spurious `-1,-1`); the client's post-commit report follows on the same ordered path and corrects it, and delta-bearing edits are cursor-gated in the window, so no corruption is constructible. Also note text-input-v3 offers no serial on client reports, so serial-based staleness detection is not an option; a content-compare against the expected echo is the upgrade path if this race ever bites.

Related invariant caveat: for Qt-style clients, the compositor's stored `state.surrounding` contains preedit-polluted text while a preedit is outstanding — see the `SurroundingText` doc comment in `compositor/src/text_input.rs` before adding consumers.
...
| Focused client commits a `set_surrounding_text` (any cause) | `onUpdateEditableText(activityId, text, selStart, selEnd, authoritative)` → matching Activity's `TawcInputConnection.updateFromCompositor(...)` — replaces Editable contents and selection AND pokes `IMM.updateSelection` so the IME drops any composing region. Ignored while the IC's own preedit is active unless `authoritative` (cause=other or enable-cycle report — see "Mid-composition echoes are ignored") |
...
- **As an app**: assertions go through `wayland-debug-app`'s observed `TAWC_DEBUG:…` events — `TEXT_CHANGED`, `PREEDIT`, `CURSOR_POS`, `KEY`, `COMMIT`, `DELETE_SURROUNDING`, `DONE`. That's what a real wayland client running under our compositor sees. The `text-input-echo-preedit` mode models Qt/KTextEditor clients that include the active preedit in surrounding-text reports.
- **As the IME's eyes**: the broker `ime-selection-updates` action dumps the `updateSelection` calls recorded by `RecordingImeOutput` — the editor→IME boundary the system IME watches. Tests use it to assert no spurious "composition ended" (`composing = -1,-1`) signal fires mid-composition, and to model the defensive `finishComposingText` a real IME issues when it does.
...
See `notes/exec-broker.md` "Registered actions" for the full action surface and `app/src/debug/java/me/phie/tawc/dev/InputActions.kt` for handler implementations.
