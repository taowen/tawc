...
helper. The bind table + chroot exec lives in Kotlin behind
[InstallationMethod.startInside] and is rebuilt fresh on every entry,
so emulator-vs-device differences (skip libhybris-only mounts on
...
- `scripts/rootfs-run.sh` — host-side chroot driver. Routes through
  the dev exec broker's `RUNINSIDE` request to
